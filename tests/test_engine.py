...
from scanner.core.engine import ScanEngine, TargetTooLargeError, expand_target
...
@pytest.mark.asyncio
async def test_host_cap_truncates_aggregate_targets():
    """Many small tokens summing past the cap are truncated, not refused."""
    scope = make_scope()
    ...
    result = await eng.run(ScanConfig(targets=["10.0.0.1", "10.0.0.2"], mode=ScanMode.NORMAL))
...
@pytest.mark.asyncio
async def test_oversized_single_token_is_refused_not_truncated():
    """One token bigger than the cap raises rather than scanning an arbitrary slice.

    This is the /8 guard: the size is checked arithmetically, so the refusal
    happens without ever materialising the address list.
    """
    scope = make_scope()
    scope.max_hosts_per_scan = 1
    eng = engine([FakeDiscoverer([])], scope=scope)
    with pytest.raises(TargetTooLargeError) as exc:
        await eng.run(ScanConfig(targets=["10.0.0.0/30"], mode=ScanMode.NORMAL))
    assert exc.value.size == 4
    assert exc.value.limit == 1


def test_expand_target_refuses_huge_cidr_without_enumerating():
    with pytest.raises(TargetTooLargeError) as exc:
        expand_target("10.0.0.0/8", limit=1024)
    assert exc.value.size == 16_777_216


def test_expand_target_refuses_huge_range():
    with pytest.raises(TargetTooLargeError):
        expand_target("10.0.0.0-10.0.255.255", limit=1024)
